...
class WorkerifyWorkerHost {
  ...
  destroy() {
    if (this.options.info)
      console.log(
        "terminating worker for worker host with id=" + this.id,
        "worker recieved " + this.calls + " calls"
      );
    this.calls = 0;
    if (this.Worker) {
  ...
  call(args) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.options.info)
        console.log("sending data to worker with id=" + this.id);
...
class WorkifyManager {
  ...
  call(args) {
    if (this.inactive) {
      throw new Error(
        "unable to call Workerify function after destruction has occured"
      );
    }
